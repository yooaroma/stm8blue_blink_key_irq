...
#define UCOM_KEY1_GPIO    GPIOD        // PD3
#define UCOM_KEY1_PIN     GPIO_PIN_3   // PD3
...
void main(void)
{
  ...
  {    // PA3
    ...
    #define EXTI_SHIFT_PORT   EXTI_SHIFT_PORTD
  ...
  }
  {
    ...
    do {
      ...
      vwDelay--;
      if(((UCOM_KEY1_GPIO->CR2 & (uint8_t)UCOM_KEY1_PIN)==0) && ((UCOM_KEY1_GPIO->IDR & UCOM_KEY1_PIN)!=0)) // jittering 방지
      {
        UCOM_KEY1_GPIO->CR2 |= (uint8_t)UCOM_KEY1_PIN; /* External interrupt enable */
      }
    } while(1);
